<div class="m-auto h-[85vmin] bg-[url('img/background.jpg')] bg-cover bg-no-repeat">
	<div class="relative m-auto h-full bg-gradient-to-t from-[#1b1b1b_5%] via-[#FFFFFF00_65%]">
		<div class="m-auto h-full w-2/3">
			<div class="flex justify-center">
				<h1 class="text-color-white font-merriweather w-[85%] pt-[20vmin] text-7xl uppercase">
					A truly outstanding coffee experience
				</h1>
			</div>
			<div>
				<p class="text-color-white m-auto w-3/4 pt-[2vmin] text-center">
					One of the most striking features of this problem is that the initial progress in the
					feedback system becomes even more complex when compared with The Bounce of Intellectual
					Software
				</p>
			</div>
			<div class="m-auto mt-[5vmin] flex w-full justify-center">
				<button
					class="text-color-white hover:bg-color-white hover:text-color-background hover:border-color-white rounded-3xl border-2 pl-6 pr-6 pt-2 pb-2 text-center duration-150"
					>Get Started</button
				>
			</div>
		</div>
	</div>
</div>
